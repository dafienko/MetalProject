...
func parseVertices(objFile: URL, materials: [String: ObjMaterial]) -> ModelGeometry {
    ...
    func parseLineV3(components: [String]) -> v3 {
        return v3(
            Float(components[1])!,
            Float(components[2])!,
            Float(components[3])!
    ...
    for line in lines {
        ...
        switch components[0] {
        case "v":
            let pos = parseLineV3(components: components)
            maxBound = v3(max(maxBound.x, pos.x), max(maxBound.y, pos.y), max(maxBound.z, pos.z))
            minBound = v3(min(minBound.x, pos.x), min(minBound.y, pos.y), min(minBound.z, pos.z))
...
extension Model: Renderable {
    func render(time: Float, encoder: MTLRenderCommandEncoder, viewMatrix: float4x4, projectionMatrix: float4x4) {
        ...
        var uniforms = Uniforms(mvMatrix: viewMatrix * transform, pMatrix: projectionMatrix)
